...
from torchvision.transforms.v2 import Compose, ToImage, ToDtype, Normalize, CenterCrop
...
class PCamDataModule(MO810DataModule):
    ...
    def __init__(self, data_dir: str = "./data/", 
        ...
        self.precomputed_dataset_stats = {'mean': torch.tensor([0.6982, 0.5344, 0.6907]), 'std': torch.tensor([0.2343, 0.2761, 0.2113])}
        ...
        if val_samples_per_class:
            self.val_subset = self.sample_dataset(self.val_subset, samples_per_class=val_samples_per_class)

        if test_samples_per_class:
            self.test_subset = self.sample_dataset(self.test_subset, samples_per_class=test_samples_per_class)
    ...
    def sample_dataset(self, dataset, fraction=None, samples_per_class=None):
        ...
        if fraction != None and samples_per_class != None:
            raise ValueError("PCamDataModule ERROR: Cannot sample dataset using both fraction and samples_per_class")
        elif fraction:
    ...
    def train_dataloader(self, fraction=None, samples_per_class=None, transform=None, shuffle=True):
        """
    ...
    def val_dataloader(self, transform=None):
        ...
        return DataLoader(dataset=TransformedSubset(subset=self.val_subset, transform=transform),
                          batch_size=self.batch_size, 
                          num_workers=self.num_workers)
